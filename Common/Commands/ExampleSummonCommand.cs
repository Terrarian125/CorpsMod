...
namespace CorpsMod.Common.Commands
{
	public class ExampleSummonCommand : ModCommand
	{
		...
		public override void Action(CommandCaller caller, string input, string[] args) {
			...
			for (int k = 0; k < numToSpawn; k++) {
				...
				int slot = NPC.NewNPC(new EntitySource_DebugCommand($"{nameof(CorpsMod)}_{nameof(ExampleSummonCommand)}"), xSpawnPosition, ySpawnPosition, type);
