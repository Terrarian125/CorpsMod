	...
	public class ExampleWorldGenHookingSystem : ModSystem
	{
		...
		void Modify_Pyramids(ILContext il) {
			try {
				...
				c.EmitDelegate(() => ModContent.GetInstance<CorpsMod>().Logger.Debug("(In ILHook) Generating Pyramids"));
			}
			catch (Exception) {
				MonoModHooks.DumpIL(ModContent.GetInstance<CorpsMod>(), il);
			}
		}
		...
		void Detour_Shinies(WorldGen.orig_GenPassDetour orig, object self, GenerationProgress progress, GameConfiguration configuration) {
			ModContent.GetInstance<CorpsMod>().Logger.Debug("(On Hook) Before Shinies");
			orig(self, progress, configuration);
			ModContent.GetInstance<CorpsMod>().Logger.Debug("(On Hook) After Shinies");
		}
	}
}
