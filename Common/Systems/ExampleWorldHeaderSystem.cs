...
namespace CorpsMod.Common.Systems
{
	public class ExampleWorldHeaderSystem : ModSystem
	{
		...
		private void DrawWorldSelectItemOverlay(UIWorldListItem uiItem, SpriteBatch spriteBatch) {
			if (MenuLoader.CurrentMenu is not CorpsModMenu)
				return;

			if (!uiItem.Data.TryGetHeaderData(this, out var data) || !data.GetBool("CorpsModExists"))
				return;
	...
	public class ExampleWorldHeaderPlayer : ModPlayer
	{
		public override void OnEnterWorld() {
			...
			// Tracking mods used to generate a world was added in v2023.8, so if WorldGenModsRecorded is false we don't know for sure if CorpsMod was enabled when this world was generated.
			if (!Main.ActiveWorldFileData.WorldGenModsRecorded) {
				return;
			}

			if (Main.ActiveWorldFileData.TryGetModVersionGeneratedWith("CorpsMod", out Version modVersion)) {
