...
namespace CorpsMod.Common.UI.ResourceOverlay
{
	public class VanillaLifeOverlay : ModResourceOverlay
	{
		...
		private void DrawClassicFancyOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = heartTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/ClassicLifeOverlay");
			context.Draw();
		}
		...
		private void DrawFancyPanelOverlay(ResourceOverlayDrawContext context) {
			...
			// "context" contains information used to draw the resource
			// If you want to draw directly on top of the vanilla hearts, just replace the texture and have the context draw the new texture
			context.texture = fancyPanelTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/FancyLifeOverlay_Panel");
		...
		private void DrawBarsOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = barsFillingTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/BarsLifeOverlay_Fill");
			context.Draw();
		}
		...
		private void DrawBarsPanelOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = barsPanelTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/BarsLifeOverlay_Panel");
