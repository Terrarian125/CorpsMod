...
namespace CorpsMod.Common.UI.ResourceOverlay
{
	public class VanillaManaOverlay : ModResourceOverlay
	{
		...
		private void DrawClassicFancyOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = starTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/ClassicManaOverlay");
			context.Draw();
		}
		...
		private void DrawFancyPanelOverlay(ResourceOverlayDrawContext context) {
			...
			// "context" contains information used to draw the resource
			// If you want to draw directly on top of the vanilla stars, just replace the texture and have the context draw the new texture
			context.texture = fancyPanelTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/FancyManaOverlay_Panel");
		...
		private void DrawBarsOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = barsFillingTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/BarsManaOverlay_Fill");
			context.Draw();
		}
		...
		private void DrawBarsPanelOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = barsPanelTexture ??= ModContent.Request<Texture2D>("CorpsMod/Common/UI/ResourceOverlay/BarsManaOverlay_Panel");
