...
namespace CorpsMod.Content.NPCs
{
	...
	public class Cmod_Tester : ModNPC
	{
		...
		private void SummonBoss(Player player, int npcType, string bossName) {
			...
			Main.NewText($"{bossName}が現れた！", 100, 200, 255);
...
//			//NPCID.Sets.FaceEmote[Type] = ModContent.EmoteBubbleType<Cmod_TesterEmote>();

//			// Influences how the NPC looks in the Bestiary
//			// ベストイアリにおけるNPCの見た目に影響します
//			NPCID.Sets.NPCBestiaryDrawModifiers drawModifiers = new NPCID.Sets.NPCBestiaryDrawModifiers() {
//				Velocity = 1f, // Draws the NPC in the bestiary as if its walking +1 tiles in the x direction
//				Direction = 1 // -1 は左、1 は右です。NPC はデフォルトで左向きに描画されますが、Cmod_Tester は右向きに描画されます。
...
//			// Set Cmod_Tester's biome and neighbor preferences with the NPCHappiness hook. You can add happiness text and remarks with localization (See an example in CorpsMod/Localization/en-US.lang).
...
//			NPC.Happiness
//				.SetBiomeAffection<ForestBiome>(AffectionLevel.Like) // Cmod_Tester prefers the forest.// 例人は森を好みます。
//				.SetBiomeAffection<SnowBiome>(AffectionLevel.Dislike) // Cmod_Tester dislikes the snow.// 例 人は雪が嫌いです。
//				.SetBiomeAffection<ExampleSurfaceBiome>(AffectionLevel.Love) // Cmod_Tester likes the Example Surface Biome// 例の人物は例の表面バイオームを気に入っています
...
//			// This creates a "profile" for Cmod_Tester, which allows for different textures during a party and/or while the NPC is shimmered.
//			// これにより、Cmod_Tester の「プロファイル」が作成され、パーティ中や NPC がきらめいている間にさまざまなテクスチャを使用できるようになります。
...
//				new FlavorTextBestiaryInfoElement("Mods.CorpsMod.Bestiary.Cmod_Tester_1"),

//				// You can add multiple elements if you really wanted to
//				// 複数の要素を追加することもできます
//				new FlavorTextBestiaryInfoElement("Mods.CorpsMod.Bestiary.Cmod_Tester_2")
//			]);
//		}
...
//		//	// NPCを直接スポーンさせる
//		//	int npcIndex = NPC.NewNPC(WorldGen.GetMainSpawnTileX() * 16, Main.worldSurface * 16, ModContent.NPCType<Cmod_Tester>());

//		//	if (npcIndex >= 0) {
//		//		// フラグも立てておく
//		//		TownNPCRespawnSystem.unlockedCmod_TesterSpawn = true;
//		//	}
//		//}
...
//			//if (TownNPCRespawnSystem.unlockedCmod_TesterSpawn)
//				{
//				// If Cmod_Tester has spawned in this world before, we don't require the user satisfying the ExampleItem/ExampleBlock inventory conditions for a respawn.
//				// Cmod_Tester が以前にこのワールドに出現したことがある場合、再出現のためにユーザーが ExampleItem/ExampleBlock インベントリ条件を満たす必要はありません。
//				return true;
//			}
...
//		// Cmod_Tester needs a house built out of CorpsMod tiles. You can delete this whole method in your townNPC for the regular house conditions.
...
//				chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.PartyGirlDialogue", Main.npc[partyGirl].GivenName));
//			}
//			// These are things that the NPC has a chance of telling you when you talk to it.
//			// これらは、NPC と会話したときに NPC が伝える可能性がある内容です。
//			chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.StandardDialogue1"));
//			chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.StandardDialogue2"));
//			chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.StandardDialogue3"));
//			chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.StandardDialogue4"));
//			chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.CommonDialogue"), 5.0);
//			chat.Add(Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.RareDialogue"), 0.1);

//			NumberOfTimesTalkedTo++;
//			if (NumberOfTimesTalkedTo >= 10) {
//				// This counter is linked to a single instance of the NPC, so if Cmod_Tester is killed, the counter will reset.
//				// このカウンターは NPC の単一インスタンスにリンクされているため、Cmod_Tester が殺されるとカウンターはリセットされます。
//				//プレイヤーがそのNPCと10回以上会話すると、NPCが特別な反応をするように設定している部分
//				chat.Add(Language.GetTextValue("１０"));
//			}
...
//			if (chosenChat == Language.GetTextValue("Mods.CorpsMod.Dialogue.Cmod_Tester.StandardDialogue4")) {
