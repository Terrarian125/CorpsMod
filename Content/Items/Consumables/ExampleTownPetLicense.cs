...
using CorpsMod.Content.NPCs.TownPets;
		...
		public static void ExampleTownPetUnlockOrExchangePet(ref bool petBoughtFlag, int npcType, string textKeyForLicense) {
			...
			if (Main.netMode == NetmodeID.MultiplayerClient) {
				if (!petBoughtFlag || NPC.AnyNPCs(npcType)) {
					...
					// The ModPacket is handled in CorpsMod.Networking.cs
					ModPacket packet = ModContent.GetInstance<CorpsMod>().GetPacket();
					packet.Write((byte)CorpsMod.MessageType.ExampleTownPetUnlockOrExchange);
					packet.Send();
				}
			}
