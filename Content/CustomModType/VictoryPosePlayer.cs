...
namespace CorpsMod.Content.CustomModType
{
	// Note: To fully understand this example, please start by reading https://github.com/tModLoader/tModLoader/tree/stable/CorpsMod/Content/CustomModType/README.md
	...
	internal class VictoryPosePlayer : ModPlayer
	{
		...
		public static void SendStartVictoryPoseMessage(int whoAmI, ModVictoryPose pose) {
			ModPacket packet = ModContent.GetInstance<CorpsMod>().GetPacket();
			packet.Write((byte)CorpsMod.MessageType.StartVictoryPose);
		...
		public static void SendCancelVictoryPoseMessage(int whoAmI) {
			ModPacket packet = ModContent.GetInstance<CorpsMod>().GetPacket();
			packet.Write((byte)CorpsMod.MessageType.CancelVictoryPose);
			packet.Write((byte)whoAmI);
			packet.Send(ignoreClient: whoAmI);
		}
	}
